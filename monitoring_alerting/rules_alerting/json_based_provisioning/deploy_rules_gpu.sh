...
deploy() {
  ...
  # back to the grep-based matcher from deploy_rules_gpu_v0_grep.sh.
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "[WARN] jq not found — falling back to grep-based title match (see deploy_rules_gpu_v0_grep.sh)"
